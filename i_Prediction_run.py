#USING MODELS TO MAKE PROBABILITY PREDICTIONS ON MICROSIMULATION DATASET
...
target_df = "#FILEPATH#/daily_urines.csv"
...
i_probs_df_overall = pd.DataFrame()
i_probs_df_overall = i_probs_df_overall.reindex(columns=['Antimicrobial', 'I', 'R', 'S','NT', 'Variable_tuning', 'micro_specimen_id', 'subject_id'])

##Making probability predictions

for antimicrobial in list(range(0, len(drops), 1)):

    prob_predict(ab_full[antimicrobial],drops[antimicrobial],to_drop,target_df,lc_abs[antimicrobial]+'i_file.pickle',lc_abs[antimicrobial]+'i_fits.pickle',
    ...
    i_probs_df_overall = pd.concat([i_probs_df_overall,probs_df_final])

i_probs_df_overall.to_csv("i_probs_df_overall.csv")

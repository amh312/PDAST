#ORGANISM ID AVAILABILITY SENSITIVITY ANALYSIS MODEL DEVELOPMENT
...
for antimicrobial in list(range(0, len(drops), 1)):
    ...
    train_test_validate_main(csv="urines5c.csv",
...
metrics_df = result_compiler(class_reps,"org_id_main_analysis_metrics.csv")
aucrocs_df = pd.DataFrame(list(aucrocs.items()), columns=['Antimicrobial', 'AUC_ROC'])
aucrocs_df['Information'] = 'Organism ID'
aucrocs_df.to_csv("org_id_aucrocs.csv", index=False)

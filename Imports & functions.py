...
def iter_stab_pred(Antimicrobial_agent):
    ...
    for i in list(range(1,101,1)):
        ...
        from sklearn.metrics import auc, f1_score, precision_score, recall_score, accuracy_score, \
            balanced_accuracy_score, classification_report, roc_auc_score, roc_curve, make_scorer, RocCurveDisplay
        from sklearn.metrics import confusion_matrix
        ...
        def lr_multi_crossval(target, cv_features, test=0.2, random=1, folds=6, C_val=0.1,
            ...
            if len(target.unique()) > 2:
                ...
                for label in vari_list:
                    ...
                    cv = RepeatedStratifiedKFold(n_splits=folds, n_repeats=10, random_state=random)
                    auc_scores = cross_val_score(log_reg, X_train, y_train, scoring='roc_auc_ovr_weighted', cv=cv,
                                                 n_jobs=-1)
                    results.append(auc_scores)
                    plt.boxplot(results, labels=["AUC-ROC"],
                                patch_artist=True,
            ...
            if len(sorted(coefs)) > 1:
                for label in best_lr.classes_:
                    ...
                    if len(coefs.sort_values(by=0, ascending=False)[0:4] != 0):
                        ...
                        bar_coef[label].plot.bar(color=line_col)
                        ax.set_xticklabels(
                            best_lr.feature_names_in_[coefs.sort_values(by=0, ascending=False)[0:4].index])
                        ax.set_title(target_ab + ":\nStrongest positive coefficients for " + label + " prediction")
                        plt.tight_layout()
                        plt.savefig(target_ab + label + "-posfeatures.pdf", format="pdf", bbox_inches="tight")
                        plt.show()
                    ...
                    if len(coefs.sort_values(by=0, ascending=True)[0:4] != 0):
                        ...
                        bar_coef[label].plot.bar(color=line_col)
                        ax.set_xticklabels(
                            best_lr.feature_names_in_[coefs.sort_values(by=0, ascending=True)[0:4].index])
                        ax.set_title(target_ab + ":\nStrongest negative coefficients for " + label + " prediction")
                        plt.tight_layout()
                        plt.savefig(target_ab + label + "-negfeatures.pdf", format="pdf", bbox_inches="tight")
                        plt.show()
        ...
        def LR_multi_final(target, final_features, test=0.2, random=1, C_val=0.1,
            ...
            if len(target.unique()) > 2:
                ...
                for label in vari_list:
                    ...
                    print('ROC_AUC score for ' + label + ': ',
                          roc_auc_score(y_test, y_pred_probs, multi_class="ovr", average=av))
...
def lr_hyp_tune_feats_t(df,target,ht_features,test=0.2,random=1,folds=6,model_type="auto",cw=None,
                      scorer='f1_weighted',target_ab='antibiotic',targ_result=''):
    X_train, X_test, y_train, y_test = train_test_split(
        ht_features,
        target,
